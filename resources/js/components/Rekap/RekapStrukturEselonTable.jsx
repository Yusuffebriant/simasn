...
// Urutan kolom eselon Pria/Wanita HARUS sama persis dengan
// App\Services\RekapService::ESELON_LIST dan
// App\Exports\RekapStrukturEselonExport::$eselonList (excel).
const ESELON_LIST = [
    "II A", "II B", "III A", "III B", "IV A", "IV B",
];

const JSON_PATH = "/rekap/struktur-eselon";
const EXPORT_PATH = "/rekap/struktur-eselon/export";
const FILENAME_PREFIX = "rekap-struktur-eselon";

function RekapStrukturEselonTable() {
    ...
    // sama dengan baris TOTAL di RekapStrukturEselonExport (sum per kolom).
    const totalPria = Object.fromEntries(ESELON_LIST.map((k) => [k, 0]));
    const totalWanita = Object.fromEntries(ESELON_LIST.map((k) => [k, 0]));
    let totalJmlPria = 0;
            ...
            <div className="flex flex-wrap items-end justify-between gap-4 mb-5">
                <div>
                    <h3 className="text-lg font-bold">
                        Rekapitulasi Pejabat Eselon
                    </h3>
                    <p className="text-sm text-gray-500">
                        Jumlah pejabat eselon diperinci menurut instansi,
                    ...
                    <table className="min-w-full text-sm border border-gray-200">
                        <thead>
                            <tr className="bg-gray-50">
                                ...
                                <th colSpan={ESELON_LIST.length} className="border px-2 py-2 text-center">PRIA</th>
                                <th rowSpan={2} className="border px-2 py-2 text-center align-bottom">JML</th>
                                <th colSpan={ESELON_LIST.length} className="border px-2 py-2 text-center">WANITA</th>
                                <th rowSpan={2} className="border px-2 py-2 text-center align-bottom">JML</th>
                                <th rowSpan={2} className="border px-2 py-2 text-center align-bottom">JML TOTAL</th>
                            </tr>
                            <tr className="bg-gray-50">
                                {ESELON_LIST.map((k, idx) => (
...
export default RekapStrukturEselonTable;