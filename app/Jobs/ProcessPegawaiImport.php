...
class ProcessPegawaiImport implements ShouldQueue
{
    ...
    public function __construct(protected ImportBatch $batch, protected string $filePath)
    {
    }
    ...
    {
        try {
            Excel::import(new PegawaiImport($this->batch), Storage::path($this->filePath));
            ...
            $this->batch->update(['status' => 'gagal']);
            throw $e; // tetap lempar supaya tercatat di failed_jobs untuk debugging
        } finally {
            // Hapus file sementara setelah selesai diproses (berhasil ataupun gagal)
